...
int main(int argc, char *argv[]) {
  std::cout << "\n>>> Memulai simulasi Wi-Fi 7 Indoor Multi-User, menunggu 30 "
               "detik... <<<\n"
            << std::endl;
  ...
  // Optimasi Agregasi A-MPDU Wi-Fi 7 untuk mencapai throughput Multi-Gigabit
  // Default Block Ack Window di ns-3 sangat kecil (64) sehingga membatasi
  // kapasitas fisik pada ~140 Mbps karena sender stall menunggu ACK. Kita
  // naikkan jadi 1024.
  ...
  // Aktifkan MIMO 4x4 untuk meningkatkan kapasitas kanal fisik Wi-Fi 7 secara
  // drastis
  ...
  // Konfigurasi Wi-Fi 7: Menggunakan lebar pita maksimum yang didukung oleh
  ...
  // 5. Setup Wifi Helper dengan Standar 802.11be (Wi-Fi 7) dan IdealWifiManager
  WifiHelper wifi;
  ...
  for (uint32_t i = 0; i < 5; ++i) {
    ...
    // DataRate. Hal ini penting di simulasi Wi-Fi 7 agar antrean MAC terisi
  ...
  std::cout << "Posisi AP          : X=4.0, Y=4.0, Z=2.9 (Langit-langit dalam "
               "ruangan)"
            << std::endl;
  std::cout << "Konfigurasi Wifi   : Wi-Fi 7 (802.11be), 160 MHz Bandwidth, 6 "
               "GHz Band, MIMO 4x4"
            << std::endl;
  std::cout << "AMPDU Aggregation  : QoS AMPDU & A-MSDU (Default ns-3 802.11be "
               "Maximum)"
            << std::endl;
