...
void MonitorQoS(Time stepTime, uint32_t totalStas) {
  // g_flowMonitor->CheckForLostPackets(); // Dihapus karena menyebabkan
  // performa ns-3 sangat lambat (hang) pada skenario ratusan ribu paket drop
  ...
  uint64_t deltaTx = currentTxPackets - g_prevTxPackets;

  if (deltaTx > 0) {
    currentPlr = ((double)deltaDrop / deltaTx) * 100.0;
    currentRel = ((double)deltaRx / deltaTx) * 100.0;
  }
  double currentMos = CalculateMOS(instDelayMs, instJitterMs, currentPlr);

  double now = Simulator::Now().GetSeconds();
  uint32_t activeStas = 0;
  if (now >= 10.0 && now < 15.0)
    activeStas = 10;
  else if (now >= 15.0 && now < 20.0)
    activeStas = 20;
  else if (now >= 20.0 && now < 25.0)
    activeStas = 30;
  else if (now >= 25.0 && now < 30.0)
    activeStas = 40;
  else if (now >= 30.0)
    activeStas = 50;
  ...
  std::cout << "[INFO] Waktu: " << now << "s | Active STAs: " << activeStas
            ...
            << " | Drop: " << deltaDrop << " | PLR: " << currentPlr << "%"
            << " | Rel: " << currentRel << "%"
            << " | MOS: " << currentMos << std::endl;

  g_csvFile << now << "," << activeStas << "," << throughputMbps << ","
            << instDelayMs << "," << instJitterMs << "," << deltaDrop << ","
...
int main(int argc, char *argv[]) {
  ...
  std::strftime(time_str_now, sizeof(time_str_now), "%Y%m%d_%H%M%S",
                std::localtime(&t_now));
  std::string timeStamp(time_str_now);

  std::string logFileName =
      "scratch/ta_wifi7/output_simulasi/terminal_output_wifi7_density_" +
      timeStamp + ".log";
  std::string csvFileName =
      "scratch/ta_wifi7/output_simulasi/hasil_density_wifi7_" + timeStamp +
      ".csv";
  ...
  g_csvFile << "Waktu(s),Active_STAs,Throughput(Mbps),Delay(ms),Jitter(ms),"
               "PacketDrop,PLR_Percent,Reliability_Percent,MOS\n";

  std::cout << "=======================================================\n";
  std::cout << "[INFO] Skenario High-Density / Kepadatan User (Wi-Fi 7)\n";
  std::cout << "======================================================="
            << std::endl;
  std::cout << "Simulasi ini dirancang untuk menguji ketahanan jaringan Wi-Fi "
               "terhadap kepadatan pengguna yang ekstrim (high-density test). "
               "Tujuannya adalah untuk mengamati seberapa baik jaringan dapat "
               "bertahan — dari sisi throughput total, keterlambatan (delay), "
               "hingga tingkat kehilangan paket (packet loss) — ketika jumlah "
               "perangkat klien terus bertambah seiring berjalannya waktu.\n\n";
  std::cout
      << "Secara konseptual, simulasi ini menggunakan sebuah Access Point (AP) "
         "utama yang memancarkan sinyal ke area ruangan. Pada detik-detik "
         "awal, hanya ada sedikit pengguna yang terkoneksi. Namun setiap "
         "beberapa detik, gelombang pengguna baru akan datang dan langsung "
         "terhubung, meminta alokasi bandwidth yang besar (150 Mbps, setara "
         "streaming Video 8K/VR) secara agresif.\n\n";
  std::cout
      << "Dengan pendekatan penambahan perangkat secara bertahap ini, kita "
         "dapat melihat titik jenuh jaringan di mana Access Point mulai "
         "kewalahan memproses antrean paket data, sehingga memberikan gambaran "
         "nyata tentang kapasitas maksimum jumlah pengguna simultan (user "
         "concurrency) yang dapat ditangani oleh Access Point Wi-Fi 7.\n";
  std::cout << "=======================================================\n";
  ...
  mobility.SetPositionAllocator(
      ...
      DoubleValue(1.0), "DeltaX", DoubleValue(1.5), "DeltaY", DoubleValue(1.5),
  ...
  for (uint32_t i = 0; i < apDevice.GetN(); ++i) {
    DynamicCast<WifiNetDevice>(apDevice.Get(i))
        ->GetPhy()
  ...
  for (uint32_t i = 0; i < staDevices.GetN(); ++i) {
    DynamicCast<WifiNetDevice>(staDevices.Get(i))
        ->GetPhy()
        ->SetMaxSupportedTxSpatialStreams(8);
    DynamicCast<WifiNetDevice>(staDevices.Get(i))
        ->GetPhy()
  ...
  // Bypass bug ns-3 GlobalRouting pada MLO dengan menggunakan Static Routing
  Ipv4StaticRoutingHelper staticRoutingHelper;

  // Server Routing
  Ptr<Ipv4> ipv4Server = serverNode.Get(0)->GetObject<Ipv4>();
  Ptr<Ipv4StaticRouting> routingServer =
      staticRoutingHelper.GetStaticRouting(ipv4Server);
  routingServer->AddNetworkRouteTo(Ipv4Address("192.168.1.0"),
                                   Ipv4Mask("255.255.255.0"),
                                   backboneInterfaces.GetAddress(1), 1);

  // AP Routing
  Ptr<Ipv4> ipv4Ap = wifiApNode.Get(0)->GetObject<Ipv4>();
  Ptr<Ipv4StaticRouting> routingAp =
      staticRoutingHelper.GetStaticRouting(ipv4Ap);
  routingAp->SetDefaultRoute(backboneInterfaces.GetAddress(0), 1);

  // STA Routing
  for (uint32_t i = 0; i < nStas; ++i) {
    Ptr<Ipv4> ipv4Sta = wifiStaNodes.Get(i)->GetObject<Ipv4>();
    Ptr<Ipv4StaticRouting> routingSta =
        staticRoutingHelper.GetStaticRouting(ipv4Sta);
    routingSta->SetDefaultRoute(apInterface.GetAddress(0), 1);
  }
  ...
  for (uint32_t i = 0; i < nStas; ++i) {
    ...
    onoff.SetAttribute("Tos",
                       UintegerValue(0xa0)); // QoS AC_VI (Video/VR Streaming)
    ...
    if (i >= 10 && i < 20)
      startTime = 15.0 + ((i - 10) * 0.02);
    else if (i >= 20 && i < 30)
      startTime = 20.0 + ((i - 20) * 0.02);
    else if (i >= 30 && i < 40)
      startTime = 25.0 + ((i - 30) * 0.02);
    else if (i >= 40)
      startTime = 30.0 + ((i - 40) * 0.02);
